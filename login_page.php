...
<body id="bo">
  <div>
    <form id="a" action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" autocomplete="off">
      <fieldset id="f">
        <legend></legend>
        <h1 style="font-family:Impact">Welcome Back!</h1>
        ...
        <p style="font-size: 50%;">Don't have an account?</p>
        <button style="color:white; font-size: 93%; font-family:Impact" type="button" class="button"
          onclick="window.location.href='signup_page.php'">SIGN UP</button>
        <br>
      </fieldset>
    </form>
    <?php
if(isset($_POST['submit']))
{
    ...
    else
    {
        ...
        $sql = "SELECT user_id, username, name FROM user WHERE username = '{$username}' AND password= '{$password}'";
