...
<body>
  ...
  if(isset($_POST['pay']))
  {
    ...
    $sql1="SELECT * FROM flight WHERE flight_id={$id};";
    $result1=mysqli_query($conn,$sql1) or die("Query Unsucessful");
    $row1=mysqli_fetch_assoc($result1);
    $sql3 = "INSERT INTO flight_booking (username,origin, destination, Passengers,Flight_id,date)
    VALUES ('{$_SESSION["username"]}','{$row1['origin']}','{$row1['destination']}',{$_SESSION['passengers']},{$id},'{$_SESSION["timestamp"]}')";
    $result3=mysqli_query($conn,$sql3) or die("Query Unsucessful");
    $sql2="SELECT flightbooking_id FROM flight_booking WHERE username='{$_SESSION["username"]}' AND Flight_id={$id} AND date='{$_SESSION["timestamp"]}'";
    $result2=mysqli_query($conn,$sql2) or die("Query Unsucessful");
    $row2=mysqli_fetch_assoc($result2);
    $sql = "INSERT INTO flight_payment (card_number,holder_name, amount, flightbooking_id)
    VALUES ('{$number}','{$name}',{$_SESSION['passengers']}*{$row1['fare']},{$row2['flightbooking_id']})";
    $result=mysqli_query($conn,$sql) or die("Query Unsucessful");
    $sql4="UPDATE flight SET noofbookings={$row1['noofbookings']}+1";
