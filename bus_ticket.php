...
<head>
    ...
    <title>Travel Mania | Bus Ticket</title>
...
<?php
    $id=$_GET['id'];
    include "config.php";
    $sql1="SELECT * FROM bus_booking WHERE busbooking_id={$id}";
    $result1=mysqli_query($conn,$sql1);
    $row1=mysqli_fetch_assoc($result1);
    $sql2="SELECT * FROM bus_payment WHERE busbooking_id={$id}";
    $result2=mysqli_query($conn,$sql2);
    $row2=mysqli_fetch_assoc($result2);
    $sql3="SELECT operator FROM bus WHERE bus_id={$row1['Bus_id']}";
    ...
    <div class="details">
        <p>
            Booking ID : <?php echo $row1['busbooking_id']?> <br><br>
