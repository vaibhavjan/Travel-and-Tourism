...
<body>
  ...
  if(isset($_POST['pay']))
  {
    ...
    $sql1="SELECT * FROM bus WHERE bus_id={$id};";
    $result1=mysqli_query($conn,$sql1) or die("Query Unsucessful");
    $row1=mysqli_fetch_assoc($result1);
  
    $sql3 = "INSERT INTO bus_booking (username,origin, destination, Passengers,Bus_id,date)
    ...
    $sql2="SELECT busbooking_id FROM bus_booking WHERE username='{$_SESSION["username"]}' AND Bus_id={$id} AND date='{$_SESSION["timestamp"]}'";
    $result2=mysqli_query($conn,$sql2) or die("Query Unsucessful");
    $row2=mysqli_fetch_assoc($result2);
    $sql = "INSERT INTO bus_payment (card_number,holder_name, amount, busbooking_id)
    VALUES ('{$number}','{$name}',{$_SESSION['passengers']}*{$row1['fare']},{$row2['busbooking_id']})";
    $result=mysqli_query($conn,$sql) or die("Query Unsucessful");
    $sql4="UPDATE bus SET noofbooking={$row1['noofbooking']}+1";
